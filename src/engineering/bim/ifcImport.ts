...
export function importIFCText(text:string):IFCImportSummary{
 ...
 for(const e of map.values()){
  const bt=mapType(e.type);if(!bt)continue;const a=splitArgs(e.args),qs=quoted(e.args),name=qs[1]||`${e.type} #${e.id}`,tag=(qs.length?qs[qs.length-1]:undefined)||`IFC-${e.id}`
