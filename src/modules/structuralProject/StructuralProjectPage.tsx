...
type Tab='project'|'geometry'|'materials'|'loads'|'combinations'|'analysis'|'design'|'foundations'|'report'
type Combo={name:string,g:number;q:number;w:number}
const tabs:[Tab,string][]=[['project','Projeto'],['geometry','Geometria'],['materials','Materiais'],['loads','Ações'],['combinations','Combinações'],['analysis','Análise'],['design','Dimensionamento'],['foundations','Fundações'],['report','Relatório']]
...
export default function StructuralProjectPage(){
 ...
 function exportJson(){const blob=new Blob([JSON.stringify(project,null,2)],{type:'application/json'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=`${name.replace(/\s+/g,'_')||'SmartStruct'}_V64.json`;a.click();URL.revokeObjectURL(u)}
 return <div className="module-page"><div className="module-head"><div><h2>Structural Project PRO · V65</h2><p>Projeto → ações → combinações → análise → pré-dimensionamento → fundações → relatório.</p></div></div>
