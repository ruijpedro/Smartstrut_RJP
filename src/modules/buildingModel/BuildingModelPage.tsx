...
export default function BuildingModelPage(){
 const[floors,setFloors]=useState(3),[bays,setBays]=useState(2),[bay,setBay]=useState(5),[storey,setStorey]=useState(3),[q,setQ]=useState(12),[wind,setWind]=useState(25),[qadm,setQadm]=useState(250)
 ...
 return <div className="module-page"><div className="module-head"><div><h2>Modelo de Edifício · V63 AUTO</h2><p>Geometria → análise global → pré-dimensionamento automático de pilares, vigas e sapatas.</p></div></div>
 <div className="work-grid"><section className="panel"><h3>Edifício</h3><div className="form-grid"><F l="Pisos" v={floors} s={setFloors}/><F l="Vãos" v={bays} s={setBays}/><F l="Largura vão" u="m" v={bay} s={setBay}/><F l="Pé-direito" u="m" v={storey} s={setStorey}/><F l="q vigas" u="kN/m" v={q} s={setQ}/><F l="Ação horizontal topo" u="kN" v={wind} s={setWind}/><F l="qadm terreno" u="kPa" v={qadm} s={setQadm}/></div></section><section className="panel"><h3>Modelo gerado</h3><BuildingSvg model={model}/>{solved.err&&<div className="solver-error">{solved.err}</div>}</section></div>
 ...
 <section className="panel"><h3>Cadeia V63</h3><div className="flow-line"><b>Edifício</b><span>→</span><b>Solver</b><span>→</span><b>Esforços</b><span>→</span><b>Secções</b><span>→</span><b>Fundações individuais</b></div><p className="note">AUTO significa pré-dimensionamento automático, não projeto regulamentar final. As regras de seleção de secção são heurísticas de engenharia; a análise matricial fornece os esforços, mas EC2/EC7, combinações, 2.ª ordem global, fissuração, punçoamento e geotecnia devem ser verificados.</p></section></>}</div>}
